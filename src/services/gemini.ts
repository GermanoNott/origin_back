...
export async function transcribeAudio(audioAsBase64: string, mimeType: string){
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: 'Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e dividida o texto em parágrafos qunando for apropriado.'
            },
            {
                inlineData: {
                    mimeType,
...
export async function generateAnswer(question: string, transcription: string[]){
    ...
    const prompt = `
        Com base no texto fornecido a baixo com contexto, responda a pergunta de forma clara e precisa em português do Brasil.
